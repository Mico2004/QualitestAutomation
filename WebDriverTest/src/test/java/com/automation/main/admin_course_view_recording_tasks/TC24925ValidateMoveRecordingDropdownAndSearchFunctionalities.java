...
public class TC24925ValidateMoveRecordingDropdownAndSearchFunctionalities {
    ...
    String noResultFoundText = "No results found";
    WebElement textArea;
    ...
    public void test24925() throws InterruptedException//
    {
        ...
        for (int login_as = 0; login_as < 2; login_as++) {
            ...
            for (int selected_tab = 0; selected_tab < 3; selected_tab++) {
                ...
                boolean isNoResultsFoundTextDisplayed = WaitDriverUtility.waitAndGetElementText(driver, members_value, noResultFoundText);

                if (isNoResultsFoundTextDisplayed) {
                    System.out.println("Dropdown list opened with the text");
                    ATUReports.add("Dropdown list opened with the text.", "Text: No results found", "Text: No results found", LogAs.PASSED, null);
                    Assert.assertTrue(true);
                } else {
                    System.out.println("Dropdown list does not contain the following text : " + noResultFoundText);
                    ATUReports.add("Dropdown list opened with the text.", "Text: No results found", "Text: " + noResultFoundText, LogAs.FAILED, null);
                ...
                textArea = driver.findElement(By.id("members_value"));
                textArea.clear();
                textArea.sendKeys(PropertyManager.getProperty("User4"));
                boolean areThisUserExist = WaitDriverUtility.waitAndGetElementText(driver, members_value, noResultFoundText);
                ...
                if (areThisUserExist) {
                    ...
                    System.out.println("Dropdown list opened with the text: " + noResultFoundText);
                    ATUReports.add("Dropdown list opened with the text.", "Text: No results found", "Text: ", LogAs.FAILED, null);
                    Assert.assertTrue(false);
                }
...
//
                ...
                textArea = driver.findElement(By.id("members_value"));
                textArea.clear();
                textArea.sendKeys("kosins1");
                boolean areThisInstructorExist = WaitDriverUtility.waitAndGetElementText(driver, members_value, noResultFoundText);


                // 18. The dropdown is displaying an informative text "No Results".

                if (areThisInstructorExist) {
                    System.out.println("Dropdown list opened with the text: No results found");
                    ATUReports.add("Dropdown list opened with the text.", "Text: No results found", "Text: No results found", LogAs.PASSED, null);
                    Assert.assertTrue(true);
                } else {
                    System.out.println("Dropdown list does not contain the expected text");
                    ATUReports.add("Dropdown list opened with the text.", "Text: No results found", "Text: ", LogAs.FAILED, null);
                    Assert.assertTrue(false);
                }
                ...
                if (existingInstructorName.equals(username)) {
                    ...
                    System.out.println("Dropdown list opened with the text: " + existingInstructorName);
                    ATUReports.add("Dropdown list opened with the text.", username, existingInstructorName, LogAs.FAILED, null);
                    Assert.assertTrue(false);
                }
