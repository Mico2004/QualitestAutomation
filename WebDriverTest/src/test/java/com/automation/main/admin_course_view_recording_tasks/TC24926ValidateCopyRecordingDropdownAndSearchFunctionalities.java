	...
	public void test24926() throws InterruptedException//
	{
		...
		for(int login_as=0; login_as<2; login_as++) {
			...
			for(int selected_tab=0; selected_tab<3; selected_tab++) {
				...
				copy.chooseInstructorAndVerifyAutoCompleteIsAsExpected("aaaaaadssdfafaasa","No results found");			
				...
				copy.chooseInstructorAndVerifyAutoCompleteIsAsExpected(PropertyManager.getProperty("User4"),"No results found");			
				
				// 17. Click on the text field, and write the name of the (existing) instuctor which exists only in another university.
				copy.chooseInstructorAndVerifyAutoCompleteIsAsExpected("kosins1","No results found");			
				
				// 19. Click on the text field, and write a sub string of the name of the (existing) instructor.
				copy.chooseInstructorAndVerifyAutoCompleteIsAsExpected(PropertyManager.getProperty("User1").substring(0, PropertyManager.getProperty("User1").length()-3),PropertyManager.getProperty("User1"));
