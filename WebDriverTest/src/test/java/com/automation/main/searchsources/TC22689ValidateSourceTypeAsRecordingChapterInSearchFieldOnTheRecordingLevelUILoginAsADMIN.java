...
//		
	...
	public void test22662() throws Exception {
		...
		// 3.Validate "Allow students to download recordings" option in "Manage
		// Course Settings" from "Courses" section is enable
		...
		String instructor = record.getIndexRecorderNameOfRecording(0);
		...
		/// 9.Validate the search field is display at the top right of the UI
		/// page below the top navigation bar.
		player_page.veriySearchBoxLocation();

		/// 10.Validate the text in the Tegrity Player page: "Search in this
		/// recording..."
		player_page.verifySearchBoxHint();

		// 11.Search the "Recording Chapter" from the recording that we
		// mentioned in the preconditions and press ENTER.

		player_page.verifySearchForRecordingExist(recording_to_search);
		player_page = PageFactory.initElements(driver, PlayerPage.class);

		/// 12.The header is displayed with the default color and the logo at
		/// the top left cornner of the UI page.
		///// to go back to crecording window handler
		...
		/// 13.The next result display below the current result in case there is
		/// next result.
		player_page.verifyThatNextResultDisplayBelowCurrentResultInCaseThereIsNextResult(player_page.search_result);

		/// 14.search results page in the format as follows: "recording name -
		/// Search Results".
		...
		/// 15.The search results on a recording level is displayed in the table
		/// with the columns as follows: "Location", "Time", "Context"
		player_page.waitForVisibility(player_page.columns_title_text.get(0));
		player_page.verifySearchColumns();

		/// 16.click on a row:The Tegrity Player page is opened and the
		/// recording start playing from the chapter start time.
		...
		/// 13.The breadcrumb structure is displayed as follows: "> Courses >
		/// course name".
