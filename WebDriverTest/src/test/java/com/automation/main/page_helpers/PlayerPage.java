...
//		}
    ...
    public boolean verifyTimeBufferStatusForXSeciFrame(int seconds) throws InterruptedException {
        try {
            (new WebDriverWait(driver, 45))
                    .until(ExpectedConditions.presenceOfElementLocated(By.id("PauseButton_Img")));
            System.out.println("Player shows pause button.");
...
//			Assert.assertTrue(false);
    ...
    public String getTimeOfBookmark(String target_bookmark) throws InterruptedException {
        ...
        try {
            ...
            if (timeReturn.contains("null")) {
                System.out.println("trying to find bookmark time recursivy");
                return getTimeOfBookmark(target_bookmark);
            }
            return timeReturn;
    ...
    public void checkThatWeStartTheRecordFromThisTime(String time) throws InterruptedException {
        ...
        getIntoFrame(0);
        int timeOut = 40;
        System.out.println("the expected time is " + time);
        while (timeOut > 0) {
//            waitForTimeBufferStatusButtonBeDisplayed();

            String currentTime = time_buffer_status.getText();
            if (currentTime != null && currentTime.equals(time)) {
                System.out.println("The record starting from the expected time");
                break;
            }
        ...
        if (timeOut == 0) {
            throw new RuntimeException("The record does not strating at bookmark");
        }
    }
