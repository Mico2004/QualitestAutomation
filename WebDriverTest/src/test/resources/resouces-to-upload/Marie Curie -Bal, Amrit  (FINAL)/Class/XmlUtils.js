...
var BT_CR		 = 4;
var sIE			 = "MSIE";
var sIE_New	 = "Trident"; 		// IE11 drops the "MSIE" token and align with the new "Trident" one.
var sFF			 = "Firefox";
var sSF			 = "Safari";
var sSF_AWK  = "AppleWebKit"; // Apple iOS WebView or UIWebView (From iOS 8) browser control.
var sCR			 = "Chrome";
...
function InitXmlFunctionsByBrowser(sUserAgent)
{
	...
	if(sUserAgent.indexOf(sIE) != -1 || sUserAgent.indexOf(sIE_New) != -1)
		BROWSER_TYPE = BT_IE;
	else if(sUserAgent.indexOf(sFF) != -1)
		BROWSER_TYPE = BT_FF;
	else if((sUserAgent.indexOf(sSF) != -1 || sUserAgent.indexOf(sSF_AWK) != -1) && (sUserAgent.indexOf(sCR) == -1))
		BROWSER_TYPE = BT_SF;
...
{	
	...
	// [2]. Second trial - XMLHttpRequest.	
	try
	{ 
		
		var oXmlReq = new XMLHttpRequest(); 
		
		oXmlReq.open("GET", sXmlPath, false); // Set to synchronous mode.
		try { oXmlReq.responseType = "msxml-document"; } catch(e) {};
		oXmlReq.send();
		
		if(oXmlReq.readyState != 4 || !(oXmlReq.status == 200 || oXmlReq.status == 0)) // Status 0, OK for local file system.
			throw new TegException(ET_XmlError, "Request completion error - [ReadyState: " + oXmlReq.readyState + ", Status: " + oXmlReq.status + "]");
			
		return oXmlReq.responseXML;
	}
	catch(e)
	{
...
function DefineSelectSingleNode_FF()
{
	...
	{ 
		// prototying the XMLDocument/Document(Chrome 34)
		...
		// ********************************
		// *** Chrome 34 bug workaround ***
		// ********************************
		if(Document && Document.prototype && typeof(Document.prototype.selectSingleNode) === "undefined")
		{
