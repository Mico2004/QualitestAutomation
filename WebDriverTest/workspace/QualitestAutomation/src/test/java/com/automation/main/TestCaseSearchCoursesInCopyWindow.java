...
public class TestCaseSearchCoursesInCopyWindow {
	...
	public void loginCourses() throws InterruptedException
	{
		...
		if(course_list_preclick_on_search_button.isEmpty()) {
			...
			ATUReports.add("Course list is not changed", LogAs.PASSED, null);
			Assert.assertTrue(true);
		} else {
			System.out.println("Course list is changed");
			ATUReports.add("Course list is changed", LogAs.FAILED, null);
		...
		if(current_course_list.size() == 0) {
			...
			ATUReports.add("Course list is empty", LogAs.PASSED, null);
			Assert.assertTrue(true);
		} else {
			System.out.println("Course list is not empty");
			ATUReports.add("Course list is not empty", LogAs.FAILED, null);
			Assert.assertTrue(false);
		}
		...
		if((current_course_list.size() == 1) && (current_course_list.get(0).equals(specific_course_title))) {
			...
			ATUReports.add("Specific cours titile is displayed", LogAs.PASSED, null);
			Assert.assertTrue(true);
		} else {
			System.out.println("Specific cours titile is not displayed");
			ATUReports.add("Specific cours titile is not displayed", LogAs.FAILED, null);
			Assert.assertTrue(false);
		}
		...
		if((is_all_courses_contain_searched_word) && (current_course_list.size() > 1)) {
			...
			ATUReports.add("Only courses with common text in titles are displayed in course list", LogAs.PASSED, null);
			Assert.assertTrue(true);
		} else {
			if ((current_course_list.size() <= 1)) {
				...
				ATUReports.add("Error - number of common  courses which displayed with same name is 1 or less", LogAs.FAILED, null);
				Assert.assertTrue(false);
			} else {
				System.out.println("Not only courses with common text in titles are displayed in course list");
				ATUReports.add("Not only courses with common text in titles are displayed in course list", LogAs.FAILED, null);
				Assert.assertTrue(false);
			}
